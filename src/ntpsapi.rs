...
use winapi::um::winnt::{
    ...
    PROCESS_MITIGATION_EXTENSION_POINT_DISABLE_POLICY, PROCESS_MITIGATION_FONT_DISABLE_POLICY,
    PROCESS_MITIGATION_IMAGE_LOAD_POLICY, PROCESS_MITIGATION_POLICY,
    PROCESS_MITIGATION_STRICT_HANDLE_CHECK_POLICY, PROCESS_MITIGATION_SYSTEM_CALL_DISABLE_POLICY,
    PSECURITY_QUALITY_OF_SERVICE,
};
use winapi_local::um::winnt::{
    PROCESS_MITIGATION_CHILD_PROCESS_POLICY, PROCESS_MITIGATION_PAYLOAD_RESTRICTION_POLICY,
...
#[inline]
pub fn PsAttributeValue(
    Number: PS_ATTRIBUTE_NUM,
    Thread: bool,
    Input: bool,
    Additive: bool,
) -> ULONG_PTR {
    (Number & PS_ATTRIBUTE_NUMBER_MASK | if Thread { PS_ATTRIBUTE_THREAD } else { 0 }
    | if Input { PS_ATTRIBUTE_INPUT } else { 0 } | if Additive { PS_ATTRIBUTE_ADDITIVE } else { 0 }
    ) as usize
}
